...
__all__  = ['r_cbam_resnet18', 'r_cbam_resnet34', 'r_cbam_resnet50', 'r_cbam_resnet101']
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.stride = stride
        self.cbam = CBAM(inplanes, planes, 16 )
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.downsample = downsample
        self.stride = stride
        self.cbam = CBAM( inplanes,planes * 4, 16 )
...
def r_cbam_resnet18(pretrained=False, **kwargs):
    model = ResidualNet('ImageNet', 18, 1000)
    return model

def r_cbam_resnet34(pretrained=False, **kwargs):
    model = ResidualNet('ImageNet', 34, 1000)
    return model

def r_cbam_resnet50(pretrained=False, **kwargs):
    model = ResidualNet('ImageNet', 50, 1000)
    return model

def r_cbam_resnet101(pretrained=False, **kwargs):
    model = ResidualNet('ImageNet', 101, 1000)
    return model


def demo():
    net = r_cbam_resnet50(num_classes=1000)
